    ...
    test('ok', () {
      ...
      var client200 = new MockClient((request) {
        if (request.url.path.endsWith("commit")) {
          var responseBody = util.readFile('test/neo4dart/service/json/findAllNodeAndRelationsById_cypherIds.json');
          return new http.Response(responseBody, 200);
        } else {
          var responseBody = util.readFile('test/neo4dart/service/json/findAllNodeAndRelationsById_batchEntities.json');
          return new http.Response(responseBody, 200);
        }
      });
      neoService.tokenFindExecutor = new TokenFindExecutor.withClient(client200);
